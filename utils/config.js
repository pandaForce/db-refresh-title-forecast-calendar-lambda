...
const APP_SOQL = { TITLED_LOTS : "select Id,  Name, pROJECT__r.Name , Stage__r.name, StatusDevr__c, LatestMasterContract__r.Name , LatestMasterContract__r.mastercontracttype__c , AllocationGroup__c , ProductAllocatedTo__c , LatestMasterContract__r.LandContractStatus__c , LatestMasterContract__r.BuildContractStatus__c , LatestMasterContract__r.PropertyContractStatus__c, LatestMasterContract__r.contractlifecycle__c, LatestMasterContract__r.contractlifecycle__r.SiteStartActual__c, LatestMasterContract__r.contractlifecycle__r.SlabStage__c,LatestMasterContract__r.contractlifecycle__r.FrameStage__c,LatestMasterContract__r.contractlifecycle__r.LockUpStage__c,LatestMasterContract__r.contractlifecycle__r.FixStage__c,LatestMasterContract__r.contractlifecycle__r.FinalStage__c ,  LatestMasterContract__r.contractlifecycle__r.SettlementStage__c , LatestMasterContract__r.ListedProduct__r.producttype__c , LandRegistrationDateActual__c , LandRegistrationTitleDate_Forecast__c , IsProjectHorizon__c from Product2 where ProductType__c = 'Land' AND Project__r.activeforreporting__c = true AND LatestMasterContract__r.MasterContractType__c != 'Land Master Contract'  AND ( ( StatusDev__c = 'Available' OR StatusDev__c = 'Withheld' )  OR (  LatestMasterContract__r.BuildContractStatus__c != 'Settled' AND LatestMasterContract__r.propertycontractstatus__c != 'Settled' )  )  AND  ( NOT Project__r.Name LIKE '%Harvest Green%' ) AND  ( NOT Project__r.Name LIKE '%Test%' ) AND ProductAllocatedTo__c != 'Build to Rent' AND LatestMasterContract__r.MasterContractType__c != 'Land and Build Master Contract' AND LatestMasterContract__r.ListedProduct__r.ProductType__c != 'Townhouse'" }
